...
abstract class ASTnode { 
    ...
    abstract public void unparse(PrintWriter p, int indent);
     ...
     //used to pass down printwriter to children
...
class DeclListNode extends ASTnode {
    ...
    public int incoffset(int start){
        for (DeclNode dnode : myDecls) {
            start = dnode.incoffset(start);
        }
...
class FnBodyNode extends ASTnode {
     ...
     */
    public void nameAnalysis(SymTable symTab, int foffset) {
        myDeclList.nameAnalysis(symTab);
        
        int offset = foffset - 8;
        offset = myDeclList.incoffset(offset);

        myStmtList.nameAnalysis(symTab);
        offset = myStmtList.incoffset(offset);

        this.localSize = foffset - offset - 8;
    ...
    //make sure there are vars
        if(localSize > 0){ 
            Codegen.generate("subu", "$sp", "$sp", localSize);
        }

        myStmtList.codeGen(p, exitLabel);
    }
...
class StmtListNode extends ASTnode {
    ...
    public void codeGen(PrintWriter p, String exitLab){
        for(StmtNode snode : myStmts){
            ...
            //special exit label
            if(snode instanceof ReturnStmtNode){
                ((ReturnStmtNode)snode).codeGen(p, exitLab);
            }
            //or else handle regualry
            else{
                snode.codeGen(p);
            }
        }
...
abstract class DeclNode extends ASTnode {
    ...
    //
    public int incoffset(int start){return start;}
...
class FnDeclNode extends DeclNode {
    ...
    public SemSym nameAnalysis(SymTable symTab) {
        ...
        sym.formalSize = typeList.size()*4;
        myBody.nameAnalysis(symTab, sym.formalSize*(-1)); // process the function body
    ...
    //offsets
    int roff = ((FnSym)myId.sym()).formalSize * (-1);
    int fpoff = roff - 4;

    // Paramters code gen
    Codegen.generateIndexed("lw", "$ra", "$fp", roff, "get ra");
    //control link
    Codegen.generate("move", "$t0", "$fp");
    
    // local variable code gen
    Codegen.generateIndexed("lw", "$fp", "$fp", fpoff, "restore FP");
    //stack pointer
    ...
    public SemSym nameAnalysis(SymTable symTab) {
        ...
        if (!badDecl) {
            try {   // add entry to symbol table
                ...
                symTab.addDecl(name, sym);
                myId.link(sym);
                sym.size = 0 - myDeclList.incoffset(0);
...
class AssignStmtNode extends StmtNode {
    ...
    public void codeGen(PrintWriter p){
        p.println("\t\t # Called fromAssignment Node");
        myAssign.codeGen(p);
    }
...
class PostIncStmtNode extends StmtNode {
    ...
    public void codeGen(PrintWriter p){
        ...
        //push added one value onto stack onto stak
        Codegen.genPop("$t0");
        ...
        //store value, where depends on global or not
...
class PostDecStmtNode extends StmtNode {
    ...
    public void codeGen(PrintWriter p){
        ...
        //store value, where depends on global or not
        SemSym sym = ((IdNode)myExp).sym();
        if(sym.global){
            Codegen.generate("sw", "$t0", "_"+((IdNode)myExp).name());
        }else{
            Codegen.generateIndexed("sw", "$t0", "$fp", (sym.offset));
        }
...
class ReadStmtNode extends StmtNode {
    ...
    public void codeGen(PrintWriter p){
        ...
        //syscall for read is 5
        Codegen.p = p;
        Codegen.generate("li", "$v0", "5");
        Codegen.generateWithComment("syscall" , "READ");

        //oush read value onto stack and adress of var and load value into adress
        Codegen.genPush("$v0");
        Codegen.generateIndexed("lw", "$t0", "$sp", 4, "peek");
...
class WriteStmtNode extends StmtNode {
    ...
    public void codeGen(PrintWriter p){
        ...
        //string types have seperate syscall number
        if(writeType instanceof IntType || writeType instanceof BoolType){
...
class IfStmtNode extends StmtNode {
    ...
    public int incoffset(int start){
        start = myDeclList.incoffset(start);
        start = myStmtList.incoffset(start);
        return start;
    }
    ...
    public void codeGen(PrintWriter p){
        ...
        //pop value and assign
        Codegen.p = p;
        Codegen.genPop("$t0");
        Codegen.generate("beq", "$t0", "0", trueLabel);

        //hande statments
        myStmtList.codeGen(p, trueLabel);

        Codegen.genLabel(trueLabel, "end of if");    
    }   
...
class IfElseStmtNode extends StmtNode {
    ...
    public int incoffset(int start){
        //have to go through all lists
        start = myThenDeclList.incoffset(start);
        start = myThenStmtList.incoffset(start);
        start = myElseDeclList.incoffset(start);
        start = myElseStmtList.incoffset(start);
        return start;
    }
    ...
    public void codeGen(PrintWriter p){
        ...
        Codegen.genLabel(doneLab, "if else done");
...
class WhileStmtNode extends StmtNode {
    ...
    public void codeGen(PrintWriter p){
        ...
        //evaluate result against condition
        Codegen.genPop("$t0");
        Codegen.generate("beq", "$t0", "0", trueLab);
...
class IdNode extends ExpNode {
    ...
    public void codeGen(PrintWriter p){
        ...
        if(mySym.global){
            ...
            Codegen.generateIndexed("lw", "$t0", "$fp", mySym.offset, "load local");
        }
...
class CallExpNode extends ExpNode {
    ...
    public void codeGen(PrintWriter p){
        ...
        // push result for other functions use
        Codegen.genPush("$v0");
    }
...
class PlusNode extends ArithmeticExpNode {
    ...
    // pop both values and add them together
        Codegen.p = p;
        Codegen.genPop("$t1");
        Codegen.genPop("$t0");
...
class AndNode extends LogicalExpNode {
    ...
    public void codeGen(PrintWriter p){
        ...
        //pop value and compare
        Codegen.p = p;
        Codegen.genPop("$t0");
        String trueLab = Codegen.nextLabel();
        String doneLab = Codegen.nextLabel();

        //do not go further if left side is false
        Codegen.generate("beq", "$t0", "0", trueLab);

        myExp2.codeGen(p);
        Codegen.generate("b", doneLab);

        //left side is false, push false/0
        Codegen.genLabel(trueLab);
        Codegen.genPush("$t0");
        Codegen.genLabel(doneLab, "And done");
    }
}
...
class EqualsNode extends EqualityExpNode {
    ...
    // string comparison
        if(myExp1 instanceof StringLitNode && myExp2 instanceof StringLitNode){
            ...
            String lhs = ((StringLitNode)myExp1).getStringVal();
            String rhs = ((StringLitNode)myExp2).getStringVal();
